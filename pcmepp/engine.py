...
from torch import optim #导入 PyTorch 优化器和学习率调度器模块。
#导入swa模块
try:
...
try:
    from adamp import AdamP, SGDP #导入 AdamP 优化器和 SGDP 优化器模块。
except ImportError:
    print('Failed to load adamp. Skip to load adamp')

from pcmepp.augmentation import Mixup #导入MSDA
from pcmepp.criterions import get_criterion #导入损失函数
from pcmepp.criterions.pcme import sample_gaussian_tensors #导入高斯采样函数
from pcmepp.models.encoders import get_image_encoder #导入图像编码器和文本编码器构造函数。
...
class PCMEPPModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        self.img_enc = get_image_encoder(
            size_augment=opt.augment.img_size_augment, **opt.model) #**opt.model，把 model 配置全部展开传入，

        self.txt_enc = get_text_encoder(**opt.model)

        #如果配置文件中指定了 torch_compile，统计编译时间花了多久
        #   default
        #   默认模式，编译开销和运行性能比较均衡。

        #   reduce-overhead
        #   尽量减少 Python 调度开销，适合小 batch / 高频调用场景，但可能更耗显存。

        #   max-autotune
        #   更激进地搜索优化策略，编译更慢，但可能运行更快。
        ...
        if opt.train.get('torch_compile'):
            ...
            if opt.train.torch_compile == 'max-autotune' or opt.train.torch_compile == 'reduce-overhead':
                ...
                torch.compile(self.img_enc) #default模式
                torch.compile(self.txt_enc)
            print(f'torch.compile takes {datetime.now() - now}')

        self.criterion = get_criterion(**opt.criterion) #损失函数，根据配置文件选择不同的损失函数
        self.hnr = HubnessNeighborRegularizer(
            opt.get('hubness_neighbor'), opt.model.embed_size)

        # See `on_train_epoch_start` for mixup_fn
        self.mixup_fn = None
        self.validation_step_outputs = {}
        self.opt = opt #保存yaml配置

        self.swa_enabled = False #是否启用SWA，默认 False
        self.eval_by_swa = False #是否在验证时使用SWA，默认 False
        if self.opt.train.get('swa_start_epoch'): #获取swa_start_epoch参数，如果有就启用
            self.swa_enabled = True
            self.swa_iter_mode = False #初始化两个更新模式
            ...
            if swa_per_iters and swa_per_epochs: #如果都设置了则报错
            ...
            if swa_per_epochs:#按 epoch 更新 SWA
            ...
            self.is_swa_criterion = is_swa_criterion #保存是否对 criterion 也做 SWA。
        ...
        self.save_hyperparameters() #Lightning 保存超参数，方便 checkpoint 记录配置。
    ...
    def forward_step(self, batch, mixup_fn=None, use_swa=False):
        ...
        if self.opt.model.precomp_enc_type == 'basic':
            ...
            if mixup_fn:
                ...
                if batch_size % 2 != 0: #batch size 是奇数，就裁掉最后一个样本，因为 Mixup 通常要把样本两两配对混合
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hnr.enabled:
            if self.opt.train.get('dist_train'):
                raise NotImplementedError(
                    'CSD-HNR currently supports single-process training only')
        ...
        if self.opt.model.is_probabilistic_model:
            ...
            loss_dict['std/img_total_var_mean'] = torch.logsumexp(
                img_emb['std'].float(), dim=-1).exp().mean()
            loss_dict['std/txt_total_var_mean'] = torch.logsumexp(
                cap_emb['std'].float(), dim=-1).exp().mean()
