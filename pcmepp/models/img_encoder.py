...
# clip_model       CLIP 模型名字，比如 ViT-B/32
# img_dim          CLIP visual 输出 token 的维度，例如 768
# embed_size       最终图像 embedding 维度，例如 1024
# no_imgnorm       是否关闭图像 embedding 的 L2 归一化
# size_augment     图像 token 随机丢弃增强比例，默认 0.2
# n_unc_layers     uncertainty layers 数量，概率模型里会用
# **kwargs         其他额外参数
...
class EncoderClipImageFull(nn.Module):
    def __init__(self, clip_model, img_dim, embed_size, no_imgnorm=False, size_augment=0.2, n_unc_layers=None, **kwargs):
        ...
        model, _ = clip.load(
            ...
            rand_init=kwargs.get('rand_init')) #_ 是返回值，这里使用一个_存储，不使用
        self.backbone = model.visual
        self.image_encoder = EncoderImageAggr(img_dim, embed_size, precomp_enc_type='backbone', no_imgnorm=no_imgnorm, **kwargs)
        self.backbone_freezed = False #是否冻结
        self.size_augment = size_augment #训练时会用它随机丢掉一部分图像 token，token 随机丢弃增强。

    #提取 CLIP visual token 特征，is_prob表示是否是概率模型，概率模型会返回 mean 和 std 两个特征。
    def forward_base(self, images, is_prob=False):
        ...
        feat_lengths = torch.zeros(base_features['mean'].size(0)).to(base_features['mean'].device) #创建一个长度向量，用来记录每张图片有多少个有效 token。base_features['mean'].size(0)是batch_size
        feat_lengths[:] = base_features['mean'].size(1)#每张图片的有效 token 数
        ...
        if self.training:
            ...
            for i in range(n_feats):
                prob = np.random.rand()
                #训练时大约 80% 的图片会尝试随机丢 token，20% 的图片保持完整 token。这是参考 VSE∞ 的实现。
                if prob > self.size_augment:
                    ...
                    if len(selected) == n_tokens: #如果保留下来的 token 数等于原 token 数，说明这次没有任何 token 被丢掉。
                        new_mean.append(base_features['mean'][i])
                        #如果是概率模型，还要把 std 特征也保留下来。
                        if is_prob:
                        ...
                        feat_lengths.append(len(selected)) #记录这张图片保留下来的有效 token 数
                else:
                    new_mean.append(base_features['mean'][i])
                    if is_prob:
                        new_std.append(base_features['std'][i])
...
#sigma_ln_init=0.01 std 分支 LayerNorm 权重的初始化值。std 分支一开始会被压得比较小，防止刚开始训练时不确定性过大。
class ProbEncoderClipImageFull(EncoderClipImageFull):
    def __init__(self, clip_model, img_dim, embed_size, no_imgnorm=False,
        ...
        if sigma_ln_init is not None:
            ...
            #std / sigma 分支的输出不能一开始太大，否则概率嵌入会不稳定；如果不用 LayerNorm 控制 std，就应该用一个很小的初始化来压住 std。
        self.scalar_uncertainty = bool(
            kwargs.get('scalar_total_variance', False))
        if self.scalar_uncertainty:
            self.scalar_variance_head = ScalarTotalVarianceHead(
                embed_size,
                embed_size,
                total_var_min=kwargs.get('total_var_min', 0.01),
                total_var_max=kwargs.get('total_var_max', 0.30),
                total_var_init=kwargs.get('total_var_init', 0.10),
            )
    ...
    def forward(self, images):
        ...
        if self.scalar_uncertainty:
            std_feat = self.scalar_variance_head(std_feat)
        features['std'] = std_feat

        return features

    def freeze_backbone(self):
        for param in self.backbone.parameters(): #CLIP 原始 visual backbone 冻结
            param.requires_grad = False

        for param in self.backbone.uncertainty_transformer.parameters(): #uncertainty_transformer 保持可训练
            param.requires_grad = True
