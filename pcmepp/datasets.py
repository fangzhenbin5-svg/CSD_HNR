...
"""
import torch
import torch.utils.data as data #导入Dataset 和 DataLoader，这是 PyTorch 标准数据读取接口。
import os
import os.path as osp #导入路径处理库。
import numpy as np
...
logger = logging.getLogger(__name__) #表示当前文件的日志记录器。__name__ 是 Python 内置变量，表示当前模块的名称。datasets.py 模块的日志记录器名称就是 pcmepp.datasets。
...
class RawImageDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenzier, opt, train, vocab_size=None, noise_ratio=None):
        ...
        self.vocab_size = vocab_size #CLIP 词表大小
        ...
        #要求 opt.model.precomp_enc_type 里面必须包含字符串 'backbone'。如果不包含，就直接报错并停止程序。
    ...
    def __getitem__(self, index):
        ...
        #如果是ViT backbone，就使用 CLIP 的 tokenizer 处理文本，并进行数据增强；否则就使用普通的 tokenizer 处理文本。
        if 'ViT' in self.backbone_source:
            target = process_caption_vit(
                clip.tokenize2(caption), self.train, self.vocab_size,  #vocab_size 是 CLIP 的词表大小。用于随机 token 替换。
            ...
            is_clip_backbone = True #告诉 collate_fn 函数，这个 batch 的文本是 CLIP 处理的
        else:
    ...
    def _process_image(self, im_in):
        ...
        if self.train:
            crop_ratio = np.random.random() * 0.4 + 0.6 #随机保留原图 60% 到 100% 的区域。
        ...
        if self.train:
            if np.random.random() > 0.5:
                processed_im = self._hori_flip(processed_im) #训练时有 50% 概率水平翻转图片。

        # Normalization
        if 'detector' in self.backbone_source:
            processed_im = self._detector_norm(processed_im) #detector 风格归一化
        else:
            processed_im = self._imagenet_norm(processed_im) #ImageNet 风格归一化。

        return processed_im #返回处理好的图片。
...
def process_caption(tokenizer, tokens, train=True):
    ...
    output_tokens = ['[CLS]'] + output_tokens + ['[SEP]'] #加 BERT 特殊符号
...
def process_caption_vit(tokens, train, vocab_size,
        ...
        if size_augment:
            ...
            if prob < size_augment and train:  # mask/remove the tokens only during training
                ...
                if prob < masking:
                    ...
                    output_tokens.append(clip._tokenizer.encoder['M']) #CLIP 原版不一定有标准 [MASK]，所以代码用了字符 'M' 的 token 近似替代。
    ...
    target = [sot] + output_tokens + [eot] #加回开头结尾 token
    target = torch.Tensor(target)
    return target

#DataLoader 的 batch 组装函数。
def collate_fn(data):
...
#构造训练集 train_loader 验证集 val_loader
def get_loader(data_path, data_name, data_split, tokenizer, opt, vocab_size=None, batch_size=100,
    ...
    # 训练时丢掉最后不满 batch 的数据。原因是 DDP 多卡训练时，不完整 batch 可能导致同步问题，也可能影响 batch 内对比学习。
    if train:
        drop_last = True
    else:
        drop_last = False
